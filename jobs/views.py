...
def perfil(request):
    ...
    if usuario.exists():
        messages.add_message(request, constants.ERROR, f'Já existe um usuário com Username {username}')
...
def enviar_projeto(request):
    ...
    if job.profissional == request.user:
    	job.arquivo_final = arquivo
    	job.status = 'AA'
    	job.save()
    	return redirect('/jobs/perfil')
    else:
    	messages.add_message(request, constants.ERROR, 'Usuário não pertence a esse job')
    	return redirect('/jobs/perfil')